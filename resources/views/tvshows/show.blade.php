...
<div class="container mx-auto px-6 py-8">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mb-8">
        <h1 class="text-3xl font-bold text-gray-900 mb-4">{{ $tvshow->title }}</h1>
        <p class="text-gray-700"><strong>Season:</strong> {{ $tvshow->season }}</p>
        <p class="text-gray-700"><strong>Year:</strong> {{ $tvshow->year }}</p>
        <p class="text-gray-700"><strong>Genres:</strong> {{ $tvshow->genres }}</p>
    </div>
    ...
    <!-- Review Form -->
    @auth
        <form action="{{ route('tvshows.reviews.store', $tvshow->id) }}" method="POST" class="bg-white p-6 rounded-xl shadow-md mb-8">
            @csrf
            <input type="hidden" name="reviewable_type" value="App\Models\TvShow">
    ...
    <!-- Reviews List -->
    <div class="space-y-6">
        @forelse($tvshow->reviews()->where('approved', true)->get() as $review)
